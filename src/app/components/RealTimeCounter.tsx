...
export function RealTimeCounter() {
  const [claimsChecked, setClaimsChecked] = useState(142);
  const [activeUsers, setActiveUsers] = useState(3);
  const [debunksToday, setDebunksToday] = useState(12);

  useEffect(() => {
    const interval = setInterval(() => {
      setClaimsChecked(prev => prev + (Math.random() > 0.7 ? 1 : 0));
      setActiveUsers(Math.floor(Math.random() * 4) + 2); // Random between 2 and 5
      if (Math.random() > 0.95) {
