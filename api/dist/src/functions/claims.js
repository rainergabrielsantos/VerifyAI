...
exports.handleClaims = handleClaims;
...
async function handleClaims(request, context) {
    context.log(`Http function processed request for url "${request.url}"`);
    try {
        await (0, db_1.connectToDatabase)();
        if (request.method === 'GET') {
            let claims = await Claim_1.Claim.find().sort({ createdAt: -1 });
            // Automatically seed database if empty
            if (claims.length === 0) {
                context.log("Database empty. Seeding initial data...");
                await Claim_1.Claim.insertMany(initialClaimsData);
                claims = await Claim_1.Claim.find().sort({ createdAt: -1 });
            }
            return { jsonBody: claims };
        }
        if (request.method === 'POST') {
            const body = await request.json();
            const newClaim = new Claim_1.Claim(body);
            await newClaim.save();
